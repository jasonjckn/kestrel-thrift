...
class KestrelthriftServiceSpec extends AbstractSpec {
  "KestrelthriftService" should {
    ...
    "set a key, multiget a key" in {
      ...
      kestrelthrift.put(testQName, List(ByteBuffer.wrap("bluebird".getBytes)))
      new String(kestrelthrift.get(testQName, false)().data.array) mustEqual "bluebird"
    }
    "fifo" in {
      kestrelthrift.flush(testQName)
      kestrelthrift.put(testQName, List(ByteBuffer.wrap("bluebird1".getBytes)))
      kestrelthrift.put(testQName, List(ByteBuffer.wrap("bluebird2".getBytes)))
      kestrelthrift.put(testQName, List(ByteBuffer.wrap("bluebird3".getBytes)))
